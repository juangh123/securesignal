...
3. Judgement (risk score / level, rebalance advice, Chinese summary):
            ...
            rebalance.append(
                {
                    ...
                    "symbol": symbol,
                    "reason": (
                        f"{symbol} 权重 {weight:.1f}% 过高，单一资产集中度风险显著，"
                        "建议分批减仓以分散风险。"
                    ),
                }
            )
            ...
            rebalance.append(
                {
                    ...
                    "symbol": symbol,
                    "reason": (
                        f"组合过度集中于 {top['symbol']}，适度增持主流资产 {symbol} "
                        "有助于降低整体波动。"
                    ),
                }
            )
            ...
            rebalance.append(
                {
                    "action": "hold",
                    "symbol": symbol,
                    "reason": f"{symbol} 权重 {weight:.1f}% 处于相对合理区间，可维持现有仓位并持续观察。",
                }
            )

    level_cn = {"low": "低风险", "medium": "中等风险", "high": "高风险"}[risk_level]
    summary = (
        f"组合总市值约 {total_value_usd:,.2f} 美元，共持有 {n} 种资产。"
        f"最大持仓为 {top['symbol']}，权重 {top['weight_pct']:.1f}%。"
        f"按集中度启发式模型评估，风险评分 {risk_score}/100，属{level_cn}。"
    )
    if concentrated:
        summary += "组合集中度偏高，建议适当降低单一资产暴露。"
    else:
        summary += "组合分散度尚可，建议按风险偏好定期再平衡。"
    summary += "（本结果由规则引擎生成。）"
...
def analyze_portfolio(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if judgement is None:
        ...
        analysis_mode = "rule-fallback"
        if llm.is_configured():
            llm_fallback_note = "（LLM 分析不可用，已回退至规则引擎。）"
